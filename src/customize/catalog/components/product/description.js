...
const ProductDescriptionComponent = (props) => {
    ...
    function openDescription() {
        const description = props.product.description ?? props.product.short_description;
        if (description) {
            let code = Identify.isRtl() ? "rtl" : "ltr";
            let html = '<html dir="' + code + '" lang=""><meta name="viewport" content="width=device-width, initial-scale=1.0"><body>' + description + "</body></html>";
    ...
    let showDescription = props.product.description || props.product.short_description;
    if (!showDescription) {
        return null;
    }
    return (
