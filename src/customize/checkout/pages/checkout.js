...
class Checkout extends SimiPageComponent {
    ...
    processCustomizePayment() {
        if (this.selectedPayment.payment_method.toLowerCase() == 'zipmoneypayment') {
            this.props.storeData('showLoading', { type: 'dialog' });
            newConnection = new NewConnection();
            newConnection.init('zipmoneypayment/standard/index', 'zippay', this, 'GET');
            newConnection.connect();
            return true;
        } else if (this.selectedPayment.payment_method.toLowerCase() == 'afterpaypayovertime') {
            this.props.storeData('showLoading', { type: 'dialog' });
            if (this.mode == checkout_mode.exist_customer) {
                newConnection = new NewConnection();
                newConnection.init('afterpay/payment/process', 'afterpay', this, 'GET');
                newConnection.connect();
            } else {
                let formdata = new FormData();
                Object.keys(this.props.data.order.billing_address).forEach(key => {
                    formdata.append(key, this.props.data.order.billing_address[key]);
                });
                formdata.append('street[0]', this.props.data.order.billing_address.street);
                formdata.append('street[1]', '');
                formdata.append('street[2]', '');
                fetch(simicart.merchant_url + 'afterpay/payment/process', {
                    method: 'post',
                    headers: {
    ...
    setData(data, requestID) {
        if (requestID == 'zippay') {
            ...
            if (data.error) {
                Toast.show({
                    text: Identify.__(data.error),
                    duration: 3000, textStyle: { fontFamily: material.fontFamily }
                });
            } else if (data.redirect_uri) {
                let params = Identify.getCheckoutParams(data.redirect_uri, this.props.quoteitems.quote_id);
                ...
                this.processCustomPayment({
                    invoice_number: '',
                    url_action: base_url + '/simiconnector/index/redirect?token=' + params.token + '&salt=' + params.salt
                });
            }
            return;
        } else if (requestID == 'afterpay') {
            this.processAfterPayResult(data);
            return;
        }
        if (this.isPlace) {
    ...
    processAfterPayResult(data) {
        this.props.storeData('showLoading', { type: 'none' });
        if (data.success == false || data.hasOwnProperty('error') || !data.token) {
            Toast.show({
                text: Identify.__(data.message),
                duration: 3000, textStyle: { fontFamily: material.fontFamily }
            });
        } else {
            let params = Identify.getCheckoutParams(this.selectedPayment.redirect_url + 'au/checkout/?token=' +
                data.token + '&relativeCallbackUrl=', this.props.quoteitems.quote_id);
            let base_url = simicart.merchant_url
            if (Identify.getMerchantConfig().storeview.base.base_url) {
                base_url = Identify.getMerchantConfig().storeview.base.base_url;
            }
