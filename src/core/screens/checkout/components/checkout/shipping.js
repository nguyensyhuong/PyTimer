            ...
            <View style={{borderBottomWidth: 0.5, borderBottomColor: '#EDEDED', paddingBottom: 10, paddingTop: 10, paddingLeft: 15, paddingRight: 15}}>
                <TouchableOpacity key={shippingMethod.s_method_code} onPress={() => {
                    ...
                    <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        <Text style={{ marginLeft: 10, color: '#c6700b', fontFamily: material.fontBold, fontSize: 16 }}>{shippingMethod.shipping_fee}</Text> :
                        <Text style={{ marginLeft: 10 }}>{Identify.formatPrice(shippingMethod.s_method_fee)}</Text>
                        }
                    </View>
                </TouchableOpacity>
                {shippingMethod.message && shippingMethod.s_method_selected ? <View style={{backgroundColor: '#eaeaea', padding: 6, marginTop: 10 }}>
                    <Text style={{fontSize: 14, lineHeight: 20, color: '#c6700b'}}>{shippingMethod.message}</Text>
                </View> : null}
            </View>
        )
    }
