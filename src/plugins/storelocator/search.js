...
class Search extends SimiPageComponent {
    ...
    componentDidMount() {
        ...
        if (!this.props.tags) {
            Connection.restData();
            Connection.connect(store_locator_tag, this, 'GET');
        }
    }
    setData(data) {
                    ...
                    <Form>
                        <TouchableHighlight onPress={() => { this.openSelect() }}>
                            <Item inlineLabel disabled pointerEvents='none'>
                                ...
                                <Input disabled pointerEvents='none' value={this.countryName} />
                            </Item>
                        </TouchableHighlight>
                        <Item inlineLabel>
                        ...
                        <View style={[styles.row, { marginTop: scale(15) }]}>
                            <Button primary style={{ marginRight: 30 }} onPress={() => { this.searchAction() }}><Text>{Identify.__('Search')}</Text></Button>
                            <Button dark onPress={() => { this.clearData() }}><Text> {Identify.__('Clear')} </Text></Button>
                        </View>
                    </Form>
                </View>

                <View style={{ paddingTop: scale(15) }}>
                    <H3>{Identify.__('Search By Tag')}</H3>
    ...
    searchAction() {
        NavigationManager.openRootPage(this.props.navigation, 'StoreLocator', {
            tag: this.tag,
            country: this.value,
            city: this.city,
            state: this.stateId,
